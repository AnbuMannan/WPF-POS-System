...
{
    public partial class BrandFormView : Window, INotifyPropertyChanged, INotifyDataErrorInfo
    {
        private readonly BrandApiService _service;
        private bool _isEdit;
        private BrandDto _editDto;
        ...
        public string BrandName
        {
            ...
            set
            {
                ...
                ClearErrors(nameof(BrandName));
                if (string.IsNullOrWhiteSpace(BrandName))
                    AddError(nameof(BrandName), "Brand name is required");
                _ = ValidateNameAsync();
            }
        }
        ...
        public BrandFormView() : this(null) { }

        public BrandFormView(BrandDto dto)
        {
            InitializeComponent();
            DataContext = this;
            ...
            try
            {
                ...
                {
                    _service = (BrandApiService)App.ServiceProvider.GetService(typeof(BrandApiService));
                }
                else
                {
        ...
        private async Task SaveAsync()
        {
            ...
            {
                BrandId = _isEdit && _editDto != null ? _editDto.BrandId : 0,
            ...
            try
            {
                var exists = await _service.CheckNameExistsAsync(BrandName, _isEdit && _editDto != null ? _editDto.BrandId : (int?)null);
                if (exists)
                {
                    AddError(nameof(BrandName), "Brand name already exists");
                    FocusFirstInvalidControl();
                    return;
                }
        ...
        private async Task ValidateNameAsync()
        {
            ...
            try
            {
                ...
                bool exists = await _service.CheckNameExistsAsync(BrandName, _isEdit && _editDto != null ? _editDto.BrandId : (int?)null);
                if (exists)
                    AddError(nameof(BrandName), "Brand name already exists");
            }
            catch
